...
class M_mahasiswa_nilai extends CI_Model {
  ...
  public function add(){
    ...
    $query = $this->db->query("SELECT * FROM tbl_mahasiswa_nilai WHERE user_id = '$user_id' and matakuliah_id = '$matakuliah_id'");
    ...
    elseif ($count == 1) {
		$array = array('user_id' => $user_id, 'matakuliah_id' => $matakuliah_id);
		$this->db->where($array);
